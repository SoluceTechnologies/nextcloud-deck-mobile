...
export async function safeWrite<T>(
  ...
  try {
    ...
    console.error(`Failed safe write operation "${operationName}":`, String(err));
    throw err;
  }
}
...
export async function safeRead<T>(
  ...
  try {
    ...
    console.log(`Completed safe read operation: ${operationName}`);
    return result;
  } catch (err) {
    console.error(`Failed safe read operation "${operationName}":`, String(err));
    throw err;
  }
}
...
export async function executeBatchedOperations<T>(
  ...
  for (let i = 0; i < batches.length; i++) {
    console.log(`Executing batch ${i + 1}/${batches.length} for ${operationName}`);
    ...
    try {
      ...
      console.error(`Failed batch ${i + 1} for ${operationName}:`, String(err));
      throw err;
    }
  }
