...
it('re-dispatches once the device comes back online', async () => {
  (useIsOnline as jest.Mock).mockReturnValue(false);
  (searchCards as jest.Mock).mockResolvedValue({ hits: [], cursor: null });
  const { rerender } = renderHook(() => useRemoteSearch('a1', 'loyer', new Set()));
  await act(async () => { jest.advanceTimersByTime(300); });
  expect(searchCards).not.toHaveBeenCalled();
  ...
  await act(async () => { jest.advanceTimersByTime(300); });
  expect(searchCards).toHaveBeenCalledTimes(1);
  expect((searchCards as jest.Mock).mock.calls[0][1]).toBe('loyer');
});
