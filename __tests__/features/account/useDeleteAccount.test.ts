...
jest.mock('@/services/nextcloud/nextcloud', () => ({ validateCredentials: jest.fn() }));
...
describe('useDeleteAccount', () => {
  ...
  it('purges local data, then drops credentials and the cached avatar', async () => {
    const order: string[] = [];
  ...
  it('keeps the account when the purge fails, so the rows stay reachable for a retry', async () => {
    ...
    expect(storage.getString('avatar:acc1')).toBe('file://a.png');
  });
});
